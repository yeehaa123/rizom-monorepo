...
While keeping the specific goal and task in mind, can you give me a minimum of ${min_num_tags} and a maximum of ${max_num_tags} single-word, simple, non-hyphenated tags for this href? Tags are valid words according to the American-English dictionary. Only include tags that are really important. Again, only give me the answer. No extra words. Tags can have no more than ${tag_length} characters, are not composed of multiple words and can thus not contain a hyphen, and are all lowercase. 
`
