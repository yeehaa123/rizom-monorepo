...
export async function checkAuthorization(authToken: string | null) {
  ...
  return authToken && publicKey && verifyAuthToken(authToken, publicKey);
}
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (origin && request.headers.get("Content-Type") === "application/json") {
    ...
    const isAuthorized = await checkAuthorization(authToken);
    const query = querySchema.parse(body);
    const data = await handleQuery(query, !!isAuthorized);
